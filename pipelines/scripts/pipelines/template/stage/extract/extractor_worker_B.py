...
class ExtractorWorkerB(ExtractorWorkersInterface):
    ...
    def _worker(self, ctx: PipelineContext) -> None:
        ...
        self._write_checkpoint(
            ...
            stage=Stage.EXTRACT,
            step=Step.PARSE,
            filename="extractor_worker_b.running.json",
        ...
        try:
            ...
            raw_dir = ctx.prepare_raw_path(pipeline=self.pipeline, subdir_format="text")
            output_file = raw_dir / "source_b.txt"
            output_file.write_text("TODO: substituir por extração real da fonte B\n", encoding="utf-8")

            self._write_checkpoint(
                ctx=ctx,
                stage=Stage.EXTRACT,
                step=Step.PARSE,
                filename="extractor_worker_b.success.json",
            ...
            self._write_checkpoint(
                ...
                stage=Stage.EXTRACT,
                step=Step.PARSE,
                filename="extractor_worker_b.failed.json",
